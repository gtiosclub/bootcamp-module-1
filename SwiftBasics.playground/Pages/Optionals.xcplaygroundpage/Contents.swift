...
guard let postCode = myPostCode else {
    fatalError()
}
myAddress += postCode
...
myAddress += myPostCode ?? "No postcode"
...
endOfGameSummary = myRank == nil ? "You did not achieve a rank in this round. Better luck next time!" : "Congrats! You've achieved rank \(myRank!) in this round."
...
guard let cookingUnwrappedMode = cookingMode else {
    foodStock -= 3
    amountOfFoodInsidePan! += 3
    print("Food added to pan.")
    fatalError()
}
...
guard let myUnwrappedString = myString else {
    fatalError()
}
length = myUnwrappedString.count
