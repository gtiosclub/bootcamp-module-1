...
if let actualPostCode = myPostCode {
    myAddress += actualPostCode
}

guard let actualPostCode = myPostCode else {
    fatalError()
} 
myAddress += actualPostCode
...
if let rankExists = myRank {
    endOfGameSummary = "Congrats! You've achieved rank \(rankExists) in this round."
} else {
    endOfGameSummary = "You did not achieve a rank in this round. Better luck next time!"
}

// Using guard let
guard let rankExists = myRank else {
    endOfGameSummary = "You did not achieve a rank in this round. Better luck next time!"
    fatalError() 
}
endOfGameSummary = "Congrats! You've achieved rank \(rankExists) in this round."
...
guard let cookMode = cookingMode else {
    cookingMode = nil
    fatalError()
}
...
if cookMode == "High" {
    cookingMode = "Medium"
} else if cookMode == "Medium" {
    cookingMode = "Low"
} else if cookMode == "Low" {
    cookingMode = nil
}

amountOfFoodInsidePan = (amountOfFoodInsidePan ?? 0) + 3
