...
myAddress += myPostCode ?? "No postcode available"
print(myAddress)
...
guard let pan = amountOfFoodInsidePan, foodStock > 0 else {
    print("I give up.")
    fatalError("No pan available or no food stock left!")
}

if let mode = cookingMode {
    switch mode {
    case "High":
        cookingMode = "Medium"
    case "Medium":
...
let foodToAdd = min(3, foodStock)
foodStock -= foodToAdd
amountOfFoodInsidePan = pan + foodToAdd

print("Food stock remaining: \(foodStock)")
print("Food inside pan: \(amountOfFoodInsidePan!)")
print("Cooking mode: \(cookingMode ?? "Off")")
