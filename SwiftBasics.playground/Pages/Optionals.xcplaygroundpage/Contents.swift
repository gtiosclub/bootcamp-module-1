...
if let unwrappedPostCode = myPostCode { myAddress += unwrappedPostCode }

guard let unwrappedPostCode = myPostCode else { fatalError() }
myAddress += unwrappedPostCode
...
if let unwrappedRank = myRank { endOfGameSummary = "Congrats! You've achieved rank \(unwrappedRank) in this round."}
else { endOfGameSummary = "You did not achieve a rank in this round. Better luck next time!" }

guard let unwrappedRank = myRank else {
    endOfGameSummary = "You did not achieve a rank in this round. Better luck next time!"
    fatalError()
}
endOfGameSummary = "Congrats! You've achieved rank \(unwrappedRank) in this round."
...
var cookingMode: String? = nil
 ...
 guard let foodInsidePan = amountOfFoodInsidePan, foodStock > 0 else {
     print("I give up.")
     fatalError()
 }
 switch cookingMode {
 ...
 foodStock -= 3
 var foodInsidePanUnwrapped = foodInsidePan + 3
